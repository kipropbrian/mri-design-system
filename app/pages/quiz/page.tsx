...
export default function QuizPage() {
  ...
  return (
    <PageContainer size="wide" className="gap-6">
      <PageHeader
      ...
      <section className="grid gap-6 border-t border-border/60 pt-6">
        <SectionHeader
          eyebrow="Performance"
          title="Practice history & species retention"
          description={`Session history (${accuracy}% lifetime accuracy) and species acoustic recall across daily rotations.`}
        />

        <Specimen label="state · tabbed practice & retention" note="shares the standard table, filters and pagination with zero layout shift">
              ...
              <TableCard
                ...
                action={
                  <TabsList>
                    ...
                    <TabsTrigger value="mastery">Species retention</TabsTrigger>
                  </TabsList>
                }
                footer={
                  <div className="flex flex-col items-center justify-between gap-3 p-3 sm:flex-row">
                    <p className="order-2 text-[0.6875rem] text-muted-foreground sm:order-1">
                      ...
                      <span className="font-medium tabular-nums text-foreground">147</span> records
                    </p>
              ...
              >
                    ...
                    <TableBody>
                      {HISTORY.map((row) => (
                        <TableRow key={row.date}>
                          <TableCell>
                            <span className="block text-xs font-medium text-foreground">{row.date}</span>
                            <span className="block text-[0.6875rem] text-muted-foreground">Daily session</span>
                          </TableCell>
                ...
                <TabsContent value="mastery">
                  <Table>
                    ...
                    <TableBody>
                      {birds.featured.slice(0, 6).map((bird, idx) => {
                        const pcts = [100, 85, 70, 55, 40, 20];
                        const pct = pcts[idx];
                        return (
                          <TableRow key={bird.speciesCode}>
                            ...
                            <TableCell className="text-right">
                              <span className="inline-flex items-center justify-end gap-2">
                                ...
                                <span
                                  className={
                                    pct >= 80
                                      ? "w-8 text-right tabular-nums text-xs text-primary-ink"
                                      : pct >= 50
                                        ? "w-8 text-right tabular-nums text-xs text-warning-ink"
                                        : "w-8 text-right tabular-nums text-xs text-destructive-ink"
                                  }
                                >
                                  {pct}%
                                </span>
                    ...
                    <TableCaption>
                      Acoustic recall retention calculated across your last 60 days of practice.
                    </TableCaption>
                  </Table>
                </TabsContent>
              </TableCard>
      ...
      <Specimen label="provenance" note="closes the quiz" className="border-t border-border/60 pt-6">
